...
class AddItemDialog(tk.Toplevel):
    ...
    def save(self):
        ...
        if not display_name:
            messagebox.            SELECT COALESCE(i.display_name, i.key) AS name, rl.qty_count, rl.qty_liters, rl.chance_percent, rl.output_slot_indexshowerror("Missing name", "Display Name is required.")
            return
...
class EditItemDialog(tk.Toplevel):
    ...
    def _reload_item_kinds(self):
        ...
        cur = self.item_kind_var.get()
        if cur not in values:
            self.item_k            SELECT COALESCE(i.display_name, i.key) AS name, rl.qty_count, rl.qty_liters, rl.chance_percent, rl.output_slot_indexind_var.set(NONE_KIND_LABEL)
        try:
            self.item_kind_combo.current(values.index(self.item_kind_var.get()))
        except Exception:
            self.item_kind_combo.current(0)
...
class EditRecipeDialog(tk.Toplevel):
    def __init__(self, app, recipe_id: int):
        ...
        self.grid_var =            SELECT COALESCE(i.display_name, i.key) AS name, rl.qty_count, rl.qty_liters, rl.chance_percent, rl.output_slot_index tk.StringVar(value=(r["grid_size"] or "4x4"))
        grid_values = ["4x4"]
